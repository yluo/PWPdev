...
CURL=filtfilt(b,a,CURL);    % and use a phase correcting implementation
ekp=CURL/rho_m/f;			% compute vert vel. (pos. down) from curl
wv=interp1(DateTime,ekp,t,'linear');     % actual curl calculated
...
taux=dt*uStress/rho_m/dz;  tauy=dt*vStress/rho_m/dz;

%if Qwinds    
    wspeed = (u10m.^2 + v10m.^2).^0.5;
%else   
%    wspeed = (uWind.^2 + vWind.^2).^0.5;
%end
%				and reiDateTime,curl,'r'nterpolate wind stress and speeds onto time vector
taux=interp1(DateTime,taux,t,'linear'); tauy=interp1(DateTime,tauy,t,'linear');
wspeed=interp1(DateTime,wspeed,t,'linear');
...
TotHeat = LHTFL + SHTFL + nLWRS + nSWRS;
ResidualHeat = csaps(DateTime,TotHeat,LowPassFactor,DateTime);  % use lowpassed heat offset
...
rhf=-nSWRS*htfact; rhf=interp1(DateTime,rhf,t,'linear');
thf=-(nLWRS + LHTFL + SHTFL)*htfact; thf=interp1(DateTime,thf,t,'linear');
hhc=-interp1(DateTime,NetHeatOffset*htfact,t,'linear');
slp=interp1(DateTime,PRES/atm_Pa,t,'linear');     % compute/interpolate sea level pressure pascals -> atmospheres
% relative humidity in mm Hg?
ph2o=RHUM/100.*Humidity(AIRT-273)/760; % compute partial pressure of water in atm
ph2o=interp1(DateTime,ph2o,t,'linear'); 
...
NetEminusPOffset=csaps(DateTime,PRATE-LHTFL/hfactor,LowPassFactor,DateTime);		% balance salt
FWFlux=FWfact * (PRATE - LHTFL/hfactor - NetEminusPOffset - EkmSaltConv); 
FWFlux=1-FWFlux/1000;			% convert to salinity multiplier (1 - kg/m^3)
FWFlux=interp1(DateTime,FWFlux,t,'linear');		% recast onto time vector
